...
async def require_admin(request):
    """Only login_code=2963 / username=.mody51777 can use admin endpoints."""
    ...
    is_admin = (
        row["login_code"] == ADMIN_USER_CODE
        or row["username"] == ".mody51777"
        or s.get("username") == ".mody51777"
    )
    if not is_admin:
        raise HTTPException(403, "Admin only")
    return s
...
async def log_callbot(username, user_id, avatar, user_item, user_val, bot_item, bot_val, won):
    ...
    embed = discord.Embed(
        title="Coinflip Ended",
        description=f"A **{db.format_value(user_val + bot_val)} value** call-bot coinflip concluded!",
...
async def startup():
    pool = await db.get_pool()
    # BUG FIX: use acquire() so schema never collides with pool-init connection
    try:
    ...
    await pool.execute("""
        ...
        WHERE in_use = TRUE
          AND id NOT IN (
              ...
              WHERE status IN ('open','processing') AND creator_inventory_id IS NOT NULL
          )
          AND id NOT IN (
    ...
    """)
    # Cancel games stuck in processing > 5 min
    await pool.execute("""
...
async def api_me(request: Request):
    ...
    # BUG FIX: also return is_admin flag so frontend can show/hide admin panel link
    row = await pool.fetchrow("SELECT login_code, username FROM users WHERE id=$1", s["user_id"])
    is_admin = (
        (row and row["login_code"] == ADMIN_USER_CODE)
        or (row and row["username"] == ".mody51777")
        or s.get("username") == ".mody51777"
    )
    return JSONResponse({
        "logged_in": True,
...
async def api_create_coinflip(request: Request):
    ...
    claimed = await pool.fetchval("UPDATE inventory SET in_use=TRUE WHERE id=$1 AND in_use=FALSE RETURNING id", inv_id)
    if not claimed:
        raise HTTPException(400, "Item is already wagered in another game")

    vs_bot = body.get("vs_bot", False)
    if vs_bot:
        # BUG FIX: coinflip vs bot MUST use bot_stock — not fabricated items
        ...
        vs_bot_item_name = await pool.fetchval("SELECT b.name FROM brainrots b WHERE id=$1", bot_item["brainrot_id"])
        ...
        try:
            ...
            user_item_name = await pool.fetchval(
                "SELECT b.name FROM brainrots b JOIN inventory i ON i.brainrot_id=b.id WHERE i.id=$1", inv_id
            ) or vs_bot_item_name or "Unknown"
            await log_callbot(caller_name, s["user_id"], caller_av or "",
                              user_item_name, float(item_value),
                              vs_bot_item_name or "Unknown", float(bot_item["value"]), won)
        except Exception:
            pass
...
async def api_join_coinflip(game_id: int, request: Request):
        ...
        async with conn.transaction():
            ...
            if item_val:
                ...
                if tax_item and float(tax_item["val"]) <= float(item_val):
                    await conn.execute("""
                        INSERT INTO bot_stock (brainrot_id, mutation_id, traits) VALUES ($1,$2,$3)
    ...
    try:
        ...
        # BUG FIX: j_val was hardcoded 0.0 — joiner bets the same item (pot), so value equals creator's item
        j_val  = c_val
        await log_coinflip(creator_name, game["creator_id"], joiner_name, s["user_id"],
                           c_item, c_val, c_item, j_val, winner_id, game["creator_side"])
    except Exception:
        pass
...
async def api_callbot(game_id: int, request: Request):
    ...
    # BUG FIX: always use bot_stock — pick within ±15%, fallback to closest
    bot_item = await pool.fetchrow("""
        SELECT s.id, s.brainrot_id, s.mutation_id, s.traits,
               ROUND(b.base_value * m.multiplier * (1 + s.traits * 0.07), 2) AS value
    ...
    if not bot_item:
        # Rollback processing status so user can try again later
        await pool.execute("UPDATE coinflip_games SET status='open' WHERE id=$1", game_id)
        raise HTTPException(400, "Bot stock is empty — add items via /admin or bot commands")
    ...
    if won:
        ...
        if tax_item and float(tax_item["val"]) <= float(wagered["value"]):
            await pool.execute("INSERT INTO bot_stock (brainrot_id, mutation_id, traits) VALUES ($1,$2,$3)",
                               tax_item["brainrot_id"], tax_item["mutation_id"], tax_item["traits"])
...
async def api_upgrade(request: Request):
    ...
    # BUG FIX: upgrader MUST use bot_stock as the target — previously values
    # were not constrained to real bot_stock entries, so "value" could be phantom
    ...
    """, body.get("stock_id"))
    if not target:
        raise HTTPException(404, "Target item not found in bot stock")
...
async def api_tip(request: Request):
    ...
    try:
        ...
        if item and from_user and to_user:
            await log_tip(from_user["username"], s["user_id"], to_user["username"], int(to_id),
...
# ─── OXAPAY WEBHOOK ───────────────────────────────────────────────────────────
# BUG FIX: original only credited on confirmations >= 3, but OxaPay sends
# status="Paid" with confirmations=0 for instant coins — handle both cases.
...
async def oxapay_webhook(request: Request):
    ...
    # BUG FIX: credit on status Paid/Completed regardless of confirmation count,
    # OR on status Confirming once we have >= 1 confirmation (was >= 3 which was wrong)
    should_credit = (
        status in ("Paid", "Completed")
        or (status == "Confirming" and confirmations >= 1)
    )
...
# ─── SABCOIN WITHDRAW ─────────────────────────────────────────────────────────
# BUG FIX: this endpoint was completely missing — 404 on any withdrawal attempt
...
async def api_sabcoin_withdraw(request: Request):
    ...
    if not currency or not address:
        raise HTTPException(400, "Currency and address are required")

    # 10% tax on withdrawals
    TAX          = 0.10
    amount_usd   = round(amount_coins / COINS_PER_USD, 2)
    tax_coins    = round(amount_coins * TAX, 2)
    net_coins    = round(amount_coins - tax_coins, 2)
    net_usd      = round(net_coins / COINS_PER_USD, 2)
    order_id     = secrets.token_hex(16)

    pool = await db.get_pool()

    # Atomically debit coins (returns None if insufficient)
    wid = await db.create_withdrawal(pool, s["user_id"], amount_coins, net_coins, tax_coins, currency, address, order_id)
    if wid is None:
        bal = await db.get_sabcoins(pool, s["user_id"])
        raise HTTPException(400, f"Insufficient balance — you have {db.format_value(bal)} SC")

    # If OxaPay is configured, initiate payout
    payout_url = ""
    if OXAPAY_MERCHANT:
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                resp = await client.post(f"{OXAPAY_API}/merchants/payout", json={
                    ...
                    "address":   address,
                    "orderId":   order_id,
                    "description": f"SabCoin withdrawal for {s['username']}",
                })
                if resp.status_code == 200:
                    d = resp.json()
                    if d.get("result") == 100:
                        await pool.execute(
                            "UPDATE sabcoin_withdrawals SET status='processing' WHERE id=$1", wid
                        )
                        payout_url = d.get("payLink", "")
        except Exception:
            pass  # Withdrawal recorded; admin can process manually
    ...
    try:
        ...
        embed = discord.Embed(title="💸 SabCoin Withdrawal Request", color=0xf59e0b)
        embed.add_field(name="User",        value=f"```{uname}```",                              inline=True)
        embed.add_field(name="Amount",      value=f"```{db.format_value(amount_coins)} SC```",   inline=True)
        embed.add_field(name="After Tax",   value=f"```{db.format_value(net_coins)} SC (${net_usd})```", inline=False)
        embed.add_field(name="Currency",    value=f"```{currency}```",                           inline=True)
...
async def api_marketplace_list(request: Request):
    ...
    body = await request.json()
    pool = await db.get_pool()
    listing_id = await db.create_listing(pool, s["user_id"], int(body.get("inventory_id")), float(body.get("price_coins", 0)))
    if not listing_id:
...
@app.delete("/api/marketplace/cancel/{listing_id}")
...
# ═══════════════════════════════════════════════════════════════════════════════
# ADMIN APIs — all require login_code=2963 / username=.mody51777
# ═══════════════════════════════════════════════════════════════════════════════

@app.get("/admin", response_class=HTMLResponse)
async def admin_page(request: Request):
    await require_admin(request)
    panel_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "templates", "admin.html")
    if not os.path.exists(panel_path):
        raise HTTPException(404, "Admin panel not found — place admin.html in templates/")
...
async def admin_stats(request: Request):
    ...
    stock_count = await pool.fetchval("SELECT COUNT(*) FROM bot_stock")
    open_flips  = await pool.fetchval("SELECT COUNT(*) FROM coinflip_games WHERE status='open'")
    return JSONResponse({"users": user_count, "bot_stock": stock_count, "open_coinflips": open_flips})
...
async def admin_list_users(request: Request):
    ...
    return JSONResponse([{
        ...
        "is_banned": r["is_banned"], "login_code": r["login_code"],
...
async def admin_brainrots(request: Request):
    ...
    return JSONResponse([{"id": r["id"], "name": r["name"], "base_value": float(r["base_value"]), "tier": r["tier"], "emoji": r["emoji"]} for r in rows])
...
async def admin_add_item(request: Request):
    ...
    return JSONResponse({"inventory_id": inv_id, "name": row["name"], "emoji": row["emoji"], "mutation": row["mutation"], "value": float(row["value"])})

# ─── ADMIN: REMOVE ITEM FROM USER ────────────────────────────────────────────

@app.delete("/api/admin/removeitem/{inventory_id}")
async def admin_remove_item(inventory_id: int, request: Request):
    await require_admin(request)
    pool = await db.get_pool()
    item = await pool.fetchrow("SELECT id FROM inventory WHERE id=$1", inventory_id)
    if not item:
        raise HTTPException(404, "Item not found")
    await db.remove_item_from_inventory(pool, inventory_id)
    return JSONResponse({"success": True})

# ─── ADMIN: ADD / REMOVE BOT STOCK ───────────────────────────────────────────
...
async def admin_add_bot_stock(request: Request):
    ...
    row      = await pool.fetchrow("""
        ...
        FROM brainrots b, mutations m WHERE b.id=$1 AND m.id=$2
    """, int(brainrot_id), int(mutation_id), traits)
    return JSONResponse({"stock_id": stock_id, "name": row["name"], "emoji": row["emoji"], "mutation": row["mutation"], "value": float(row["value"])})
...
# ─── ADMIN: CREATE COINFLIP (multi-item, admin-only) ─────────────────────────
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            items = await conn.fetch("""
                ...
                JOIN brainrots b ON i.brainrot_id = b.id
                JOIN mutations m ON i.mutation_id = m.id
                WHERE i.id = ANY($1::int[]) AND i.user_id=$2 AND i.in_use=FALSE
            """, inventory_ids, int(user_id))
            if len(items) != len(inventory_ids):
                raise HTTPException(400, "Some items not found, wrong user, or already in use")
            await conn.execute("UPDATE inventory SET in_use=TRUE WHERE id=ANY($1::int[])", inventory_ids)
            game_id = await conn.fetchval("""
                INSERT INTO coinflip_games (creator_id, creator_inventory_id, creator_side)
                VALUES ($1,$2,$3) RETURNING id
            """, int(user_id), inventory_ids[0], side)
    total = sum(float(i["value"]) for i in items)
    return JSONResponse({"game_id": game_id, "total_value": total, "item_count": len(inventory_ids), "side": side})

# ─── ADMIN: BAN / UNBAN / TIMEOUT USER ────────────────────────────────────────

@app.post("/api/admin/ban/{user_id}")
async def admin_ban_user(user_id: int, request: Request):
    await require_admin(request)
    pool = await db.get_pool()
    await pool.execute("UPDATE users SET is_banned=TRUE WHERE id=$1", user_id)
    return JSONResponse({"success": True})
...
async def admin_unban_user(user_id: int, request: Request):
    await require_admin(request)
    pool = await db.get_pool()
    await pool.execute("UPDATE users SET is_banned=FALSE WHERE id=$1", user_id)
    return JSONResponse({"success": True})

@app.post("/api/admin/timeout/{user_id}")
async def admin_timeout_user(user_id: int, request: Request):
    await require_admin(request)
    body    = await request.json()
