...
# ─── POOL INIT ────────────────────────────────────────────────────────────────
# FIX: No more global singleton _pool. The pool is created once in bot's
# setup_hook (correct event loop) and passed around explicitly.
# server.py creates its own pool on startup via init_pool() as well.

async def init_pool() -> asyncpg.Pool:
    """Create and return a fresh asyncpg pool. Call this inside an async context
    that already belongs to the correct event loop (e.g. setup_hook or lifespan)."""
...
async def add_to_bot_stock(pool: asyncpg.Pool, brainrot_id: int, mutation_id: int, traits: int = 0) -> int:
    return await pool.fetchval("""
...
async def create_coinflip(pool: asyncpg.Pool, creator_id: int, inventory_id: int, side: str) -> int:
    return await pool.fetchval("""
...
async def cancel_coinflip(pool: asyncpg.Pool, game_id: int):
    ...
    if game and game["creator_inventory_id"]:
        await pool.execute("UPDATE inventory SET in_use=FALSE WHERE id=$1", game["creator_inventory_id"])
...
async def create_ticket(pool: asyncpg.Pool, user_id: int, ticket_type: str, channel_id: int) -> int:
    return await pool.fetchval("""
...
async def send_tip(pool, from_id: int, to_id: int, inventory_id: int) -> dict:
    """Transfer an inventory item as a tip. Returns success/reason dict."""
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute("UPDATE inventory SET user_id=$1, in_use=FALSE WHERE id=$2", to_id, inventory_id)
            await conn.execute(
